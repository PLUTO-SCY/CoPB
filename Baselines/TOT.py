...
parser.add_argument('--TemplateRes_PATH', default = 'TemplateRes_TOT', type=str)
        ...
        for index in range(loopStart, loopEnd): 
            ...
            for i in range(N):
                ...
                for i in range(3):      
                    ...
                    Candidate_intentions.append(answer)
                
                chooseOK = False
                while not chooseOK:
                    historyPrompts = "The arrangement of the previous {} things is as follows: {}. ".format(i, history) if i==0 else ''
                    chooseOne_question = "{} Now I want you to decide what your next intent is, and there are three options: {}. I hope you can choose the most suitable one. Just answer the choice please. No explanation is required.".format(historyPrompts, Candidate_intentions)
                    QQ = [{"role": "user", "content": chooseOne_question}]
                    nowQQ = globalInfo + QQ
                    choose_response = askChatGPT(nowQQ, model=GPT_MODEL, temperature = 1)
                    choose_response = choose_response.strip()
                    if choose_response in ["go to work", "go home", "eat", "do shopping", "do sports", "excursion", "leisure or entertainment", "go to sleep", "medical treatment", "handle the trivialities of life", "banking and financial services", "cultural institutions and events"]: 
                        chooseOK = True

                answer = choose_response
