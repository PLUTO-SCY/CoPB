...
class Unet1D_condition(Module):  # 其实是属于比较轻量级的修改方式
    def __init__(
        ...
        self.channels = channels
        self.self_condition = self_condition
        # This part is for input channels; after the init_conv, we'll concatenate, so we need to account for that
        input_channels = channels * (1 if not self_condition else 2)

        # Initial convolution layer - no changes needed here
        init_dim = default(init_dim, dim)
        self.init_conv = nn.Conv1d(input_channels, init_dim, 7, padding = 3)
        ...
        for ind, (dim_in, dim_out) in enumerate(in_out):
            ...
            # Now we need to adjust for the doubled input channels after init_conv and concatenation
            self.downs.append(ModuleList([
                resnet_block(dim_in, dim_in),
                resnet_block(dim_in, dim_in),
                Residual(PreNorm(dim_in, LinearAttention(dim_in))),
        ...
        for ind, (dim_in, dim_out) in enumerate(reversed(in_out)):
            ...
            # Adjust for doubled input channels after concatenation
            self.ups.append(ModuleList([
                resnet_block(dim_out + dim_in, dim_out),
                resnet_block(dim_out + dim_in, dim_out),
                Residual(PreNorm(dim_out, LinearAttention(dim_out))),
        ...
        # Output dimension (based on learned_variance flag)
        default_out_dim = channels * (1 if not learned_variance else 2)
        self.out_dim = default(out_dim, default_out_dim)

        # Final layers (no change here)
        self.final_res_block = resnet_block(init_dim * 2, init_dim)
        self.final_conv = nn.Conv1d(init_dim, self.out_dim, 1)

        # Embedding layer for category
        self.embCate_layer = nn.Embedding(num_embeddings=numCategory, embedding_dim=dim)
    ...
    def forward(self, x, time, emb, x_self_cond = None):
        ...
        cateEmbedding = self.embCate_layer(emb).transpose(1,2)
        x = torch.cat([x, cateEmbedding], dim=1)

        r = x.clone()
        
        # batchsize:500 length:144 dim:64
        # x.shape: torch.Size([500, 64, 144])
        # cateEmbedding.shape: torch.Size([500, 144, 64])
        # r.shape:  torch.Size([500, 64, 144])
        # t.shape:  torch.Size([500, 256])
...
class GaussianDiffusion1D(Module):
    ...
    def model_predictions(self, x, t, intent, x_self_cond = None, clip_x_start = False, rederive_pred_noise = False):
        model_output = self.model(x, t, intent, x_self_cond)
    ...
    def p_mean_variance(self, x, t, intent, x_self_cond = None, clip_denoised = True):
        preds = self.model_predictions(x, t, intent, x_self_cond)
    ...
    @torch.no_grad()
    def p_sample(self, x, t: int, intent = None, x_self_cond = None, clip_denoised = True):
        b, *_, device = *x.shape, x.device
        batched_times = torch.full((b,), t, device = x.device, dtype = torch.long)
        model_mean, _, model_log_variance, x_start = self.p_mean_variance(x = x, t = batched_times, intent = intent, x_self_cond = x_self_cond, clip_denoised = clip_denoised)
    ...
    @torch.no_grad()
    def sample(self, intent, batch_size = 16):
        seq_length, channels = self.seq_length, self.channels
        sample_fn = self.p_sample_loop if not self.is_ddim_sampling else self.ddim_sample
        return sample_fn((batch_size, channels, seq_length), intent)
