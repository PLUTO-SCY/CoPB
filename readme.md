...
- **Dataset generated by CoPB:** link: https://drive.google.com/drive/folders/1V7FIitnuC4w6b4INXcBItYKgq4z97moB?usp=drive_link without password
...
**More detailed explanation of the dataset generated by CoPB:**

The dataset generated by CoPB contains a total of **200,000** user trajectories, comprising **794,820** trajectory points in total. It is designed to support both behavioral analysis and spatial-temporal pattern mining, enabling researchers to explore user movement dynamics, mobility patterns, and contextual decision-making processes in real-world scenarios. Each trajectory is a sequence of user activities, annotated with intent, timestamps, place information, and geographic coordinates.

To support diverse usage scenarios, we provide the dataset in two formats:

- JSONL (`.jsonl`): Each line represents a full trajectory (i.e., a list of places with metadata), allowing line-by-line streaming or text-based parsing.
- Parquet (`.parquet`): The same data flattened into tabular format, with each row representing a single point and a `trajectory_id` field to group records by trajectory. This format is optimized for large-scale analytics and ML pipelines.

**Load JSONL (one trajectory per line):**
...
def load_jsonl(file_path):
    ...
    return trajectories
trajectories = load_jsonl('trajectory_CoPB.jsonl')
print(f"Loaded {len(trajectories)} trajectories.")
```

**Load Parquet (flattened table with `trajectory_id`):**

```python
import pandas as pd
df = pd.read_parquet('trajectory_CoPB.parquet')
print(df.head())
# Group by trajectory if needed:
grouped = df.groupby('trajectory_id')
for traj_id, traj_df in grouped:
    print(f"Trajectory {traj_id} has {len(traj_df)} points.")
```

**Notes:**

- All timestamps are in `HH:MM` 24-hour format.
- Place coordinates are in standard WGS84 (longitude, latitude).
